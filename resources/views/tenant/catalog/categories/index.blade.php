...
@section('content')
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500">Organize products by category.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            ...
            <form method="POST" action="{{ route('tenant.categories.store', $tenant) }}" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="parent_id" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 mb-4">
                <form method="GET" action="{{ route('tenant.categories.index', $tenant) }}" class="flex gap-3">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search category..." class="w-full rounded-lg border border-slate-300 px-3 py-2">
                                ...
                                <td class="px-4 py-3">
                                    <details class="relative">
                                        ...
                                        <div class="absolute right-0 z-10 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-lg p-4">
                                            <form method="POST" action="{{ route('tenant.categories.update', [$tenant, $category->id]) }}" class="space-y-3">
                                                @csrf
                                                @method('PUT')

                                                <input type="text" name="name" value="{{ $category->name }}" required class="w-full rounded-lg border border-slate-300 px-3 py-2">

                                                <select name="parent_id" class="w-full rounded-lg border border-slate-300 px-3 py-2">
                                                    <option value="">No parent</option>
                                                    @foreach($parentCategories as $parentCategory)
                                                        @if($parentCategory->id !== $category->id)
                                                            <option value="{{ $parentCategory->id }}" @selected($category->parent_id === $parentCategory->id)>
                                                                {{ $parentCategory->name }}
                                                            </option>
                                                        @endif
                                                    @endforeach
                                            ...
                                            <form method="POST" action="{{ route('tenant.categories.destroy', [$tenant, $category->id]) }}" class="mt-3" onsubmit="return confirm('Delete this category?')">
                                                @csrf
                                                @method('DELETE')
